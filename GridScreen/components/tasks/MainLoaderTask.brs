...
sub GetContent()
    ' rewuest the content feed from the API
    ...
    if json <> invalid
        for each category in json
            ...
            ' set up a root ContentNode rowList on the GridScreen
            contentNode = CreateObject("roSGNode", "ContentNode")
            contentNode.Update({
                children: rootChildren
            }, true)
            ' populate content field with root content node.
            ' Observer(see OnMainContentLoaded in MainScene.brs) is involed at that moment
    ...
    function GetItemData(video as Object) as Object
        item = {}
        ' populate some standart content metadata fields to be displayed on the GridScreen
        ...
        if video.longDescription <> invalid
            item.description = video.slongDescription
        else
