sub ShowVideoScreen(content as Object, itemIndex as Integer)
    m.videoPlayer = CreateObject("roSGNode", "Video") ' create a new instance of video node for each playback
    ...
    if itemIndex <> 0 ' check if user select any but first item of the row
        ...
        ' populate children array only with itemd started from selected one.
        ...
        childrenClone = []
        ' go through rach item of children array and clone them
        for each child in children
        ' we need to clone item node because ot woll be damaged in case of video node content invalidation
...
sub OnVideoVisibleChange() ' invoked when video node visibility is changed
    if m.videoPlayer.visible = false and m.top.visible = true
        ...
        ' clear video player cintentm for proper start of bext video player
